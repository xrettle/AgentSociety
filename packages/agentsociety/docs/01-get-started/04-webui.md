...
可视化界面的启动与{ref}`命令行运行 <cli>`中所使用的配置文件中的`env`字段具有相同的配置项。
...
AgentSociety可视化界面中配置的各项内容，与编程使用方式中的配置项具有相同的含义，用户可以参考[实验配置](../03-config/index.md)了解各项配置的含义。
```
...
2. 循环以下的流程
   - 添加步骤
   - 根据实验需求选择相应的步骤类型
   - 配置节点参数：根据所选步骤类型填写相应的配置参数，具体字段含义参考{ref}`实验配置-工作流步骤类型 <workflowstep>`文档
3. 保存
...
- `查看日志`：查看实验的运行日志
- `导出产物`：导出通过`save_context`机制保存的数据，该机制主要通过编写特定的代码实现，也可通过{ref}`实验配置-保存智能体上下文 <save_context>`触发。
