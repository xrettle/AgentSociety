...
export function parseDisabledToolsEnv(raw: string | undefined): string[] {
  if (!raw?.trim()) {
    return [];
  }
  return raw
    .split(',')
    .map((s) => s.trim())
    .filter((name) => PERSON_AGENT_DISABLEABLE_TOOLS.includes(name));
}

export function formatDisabledToolsEnv(names: string[] | undefined): string | undefined {
  if (!names?.length) {
    return undefined;
  }
  const filtered = names.filter((n) => PERSON_AGENT_DISABLEABLE_TOOLS.includes(n));
